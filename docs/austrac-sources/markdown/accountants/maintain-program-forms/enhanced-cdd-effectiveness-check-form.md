# Enhanced customer due diligence (CDD) effectiveness check

This form must be completed by the AML/CTF compliance officer quarterly (or less frequently if operating effectively) and whenever issues or errors are identified.

#### Section 1: Review details

| Review period (quarter/date range): |  |
| --- | --- |
| Reviewed by: |  |
| Date: |  |

#### Section 2: Review planning

Define scope

Select which high risk factors you will review this period:

| Risk factor | Selection |
| --- | --- |
| High risk of money laundering or terrorism finance (ML/TF) | [ ] |
| Link to high risk or Financial Action Task Force (FATF)-monitored country | [ ] |
| High risk domestic or international organisation politically exposed person (PEP) | [ ] |
| Foreign PEP | [ ] |
| Unusual or inconsistent activity | [ ] |
| Suspicious matter report (SMR) previously submitted | [ ] |

Sampling method

Describe how you selected the files. For example, random, risk-based or specific timeframe.

| Record details here: |
| --- |

File selection

Select at least 3 high risk client files in each category for testing.

| Type | Client /file name | Reason for selection (if applicable) |
| --- | --- | --- |
| High ML/TF risk |  |  |
| Link to high risk country |  |  |
| High risk domestic or international organisation PEP |  |  |
| Foreign PEP |  |  |
| Unusual or inconsistent activity |  |  |
| SMR previously submitted |  |  |

#### Section 3: Testing

Verify completeness

For each high risk client selected, confirm all required information is recorded. List any missing or incomplete details.

| Checkpoint | Completed | Missing/incorrect details | Evidence/notes |
| --- | --- | --- | --- |
| Source of wealth | [ ]   Yes     [ ]   No |  |  |
| Source of funds | [ ]   Yes     [ ]   No |  |  |
| Open-source background checks | [ ]   Yes     [ ]   No |  |  |
| Ownership structure diagram | [ ]   Yes     [ ]   No |  |  |
| Other (specify): | [ ]   Yes     [ ]   No |  |  |

Verify timelines

Check the date the client was escalated to the AML/CTF compliance officer. Identify any late submissions and explanations.

| Record identification | Escalation date | Submission date | Within timeframe? | Comments/reasons for delay |
| --- | --- | --- | --- | --- |
|  |  |  | [ ]   Yes<br>[ ]   No |  |
|  |  |  | [ ]   Yes<br>[ ]   No |  |
|  |  |  | [ ]   Yes<br>[ ]   No |  |

Verify submission accuracy

Compare enhanced customer due diligence (ECDD) file data with internal system records. Record any inconsistencies and required corrections below.

| Record ID | Data fields checked | Error(s) found? | Description of error(s) and action taken |
| --- | --- | --- | --- |
|  |  | [ ]   Yes<br>[ ]   No |  |
|  |  | [ ]   Yes<br>[ ]   No |  |
|  |  | [ ]   Yes<br>[ ]   No |  |

Verify follow-up actions

Check post-ECDD actions are completed. For example, client risk updated or alerts reviewed.

| Client file name | Follow-up action required | Completed? | Date completed | Comments/further actions |
| --- | --- | --- | --- | --- |
|  |  | [ ]   Yes<br>[ ]   No |  |  |
|  |  | [ ]   Yes<br>[ ]   No |  |  |
|  |  | [ ]   Yes<br>[ ]   No |  |  |

Verify senior manager approval

Confirm senior manager approval is documented for each high risk client.

| Client file name | Date escalated | Date approved by senior manager | Was approval granted before providing or continuing services? |
| --- | --- | --- | --- |
|  |  |  | [ ]   Yes<br>[ ]   No |
|  |  |  | [ ]   Yes<br>[ ]   No |
|  |  |  | [ ]   Yes<br>[ ]   No |

#### Section 4: Personnel escalation

Check if personnel are following the escalation process correctly.

| Staff member | Client file name/escalation | All information provided? | Escalated on time? | Training or communication issues identified |
| --- | --- | --- | --- | --- |
|  |  | [ ]   Yes<br>[ ]   No | [ ]   Yes<br>[ ]   No |  |
|  |  | [ ]   Yes<br>[ ]   No | [ ]   Yes<br>[ ]   No |  |
|  |  | [ ]   Yes<br>[ ]   No | [ ]   Yes<br>[ ]   No |  |

#### Section 5: Findings and corrective actions

Follow the Maintain your AML/CTF program form when implementing corrective actions and changes to the AML/CTF program.

If a corrective action fails, review and update it as soon as practicable.

| Finding | Corrective action required | Responsible person | Target date | Date completed | Approved by senior manager? |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  | [ ]   Yes<br>[ ]   No |
|  |  |  |  |  | [ ]   Yes<br>[ ]   No |
|  |  |  |  |  | [ ]   Yes<br>[ ]   No |

#### Section 6: Communication and training outcomes

List any process or training updates required following this review.

| Update/training need | Responsible person | Due date | Status/notes |
| --- | --- | --- | --- |
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |

#### Section 7: Sign-off

Submit the report to the senior manager and governing body for review and sign-off.

| Name | Role | Date |
| --- | --- | --- |
|  | AML/CTF compliance officer |  |
|  | Senior manager |  |
|  | Governing body representative (if applicable) |  |
